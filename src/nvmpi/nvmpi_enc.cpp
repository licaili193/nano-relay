...
#define CHUNK_SIZE 640*480*3/2
#define MAX_BUFFERS 1
#define TEST_ERROR(condition, message, errorCode)    \
	if (condition)                                   \
{                                                    \
	std::cout << "MVMPI Error: " << message;         \
}
...
nvmpictx* nvmpi_create_encoder(nvCodingType codingType,nvEncParam * param){
	...
	ctx->buf_index=0;
	ctx->enable_extended_colorformat = param->use_extend_color_format;
	...
	ctx->virtual_buffer_size = param->vbv_size;
	...
	// if(codingType == NV_VIDEO_CodingHEVC){
		// Hard code level here
	// 	ret = ctx->enc->setLevel(V4L2_MPEG_VIDEO_H265_LEVEL_1_0_MAIN_TIER);
	//	TEST_ERROR(ret < 0, "Could not set encoder level", ret);
	// }
	...
	// TESTING OPTIONS
	// ret = ctx->enc->setSliceLength(V4L2_ENC_SLICE_LENGTH_TYPE_BITS, ctx->bitrate / ctx->fps_n);
	// TEST_ERROR(ret < 0, "Could not setup slice length", ret);
	// ret = ctx->enc->setSliceLevelEncode(true);
	// TEST_ERROR(ret < 0, "Could not setup slice level", ret);
